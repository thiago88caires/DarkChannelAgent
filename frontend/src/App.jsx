...
import DraftVideos from './pages/DraftVideos.jsx'
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 960, margin: '0 auto', padding: 16 }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <nav style={{ display: 'flex', gap: 12 }}>
          ...
          {session && <Link to="/create">Criar Videos</Link>}
          {session && <Link to="/videos">Meus Videos</Link>}
          {session && <Link to="/videos/drafts">Rascunhos</Link>}
          {session && <Link to="/youtube">YouTube</Link>}
          {session && <Link to="/buy">Comprar Creditos</Link>}
      ...
      <main style={{ marginTop: 24 }}>
        <Routes>
          ...
          <Route path="/videos/drafts" element={<Protected><DraftVideos /></Protected>} />
